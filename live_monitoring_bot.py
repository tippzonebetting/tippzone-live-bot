...
class LiveMonitoringBot:
    ...
    def get_live_matches(self) -> List[Dict]:
        """Get live matches from SportMonks API with improved includes"""
        try:
            url = f"{self.base_url}/livescores/inplay"
            params = {
                'api_token': self.api_token,
                # FIXED: Added 'scores' to get fresh score data
                'include': 'participants,state,statistics,events,league,scores',
    ...
    def extract_team_names(self, match: Dict) -> tuple:
        """Extract home and away team names with improved logic"""
    ...
    def extract_score(self, match: Dict) -> tuple:
        """Extract current score with improved logic using scores data"""
        # FIXED: Try to get score from 'scores' first (most accurate)
        ...
        if scores:
            ...
            # If we found scores data, return it
            if 'home_score' in locals() and 'away_score' in locals():
                return home_score, away_score
        
        # Fallback to participants meta (old method)
        participants = match.get('participants', [])
        home_score = 0
        away_score = 0
    ...
    def get_player_team_mapping(self, match: Dict) -> Dict[str, str]:
        """Create a mapping of player names to team names"""
        player_team_map = {}
        
        # Try to get lineups first (most accurate)
        ...
        # If no lineups, try to extract from participants and events
        if not player_team_map:
            ...
            for participant in participants:
                ...
                location = meta.get('location')
                name = participant.get('name', 'Ismeretlen')
                team_names[location] = name
            
            # Map events to teams based on participant_id
            events = match.get('events', [])
            for event in events:
                player_name = event.get('player_name')
                participant_id = event.get('participant_id')
                
                if player_name and participant_id:
                    # Find which team this participant_id belongs to
                    for participant in participants:
                        if participant.get('id') == participant_id:
                            meta = participant.get('meta', {})
                            location = meta.get('location')
                            team_name = team_names.get(location, 'Ismeretlen')
                            player_team_map[player_name] = team_name
                            break
    ...
    def extract_events(self, match: Dict) -> List[Dict]:
        """Extract match events with improved team mapping"""
        events = match.get('events', [])
        processed_events = []
        
        # Get player-team mapping
        player_team_map = self.get_player_team_mapping(match)
        
        for event in events:
            player_name = event.get('player_name', 'Ismeretlen')
            
            # FIXED: Use the player-team mapping to get correct team name
            team_name = player_team_map.get(player_name, 'Ismeretlen')
    ...
    def generate_goal_message(self, match: Dict, event: Dict) -> str:
        """Generate goal message with improved format"""
        ...
        # FIXED: Improved message format with proper score display
        message = f"⚽ GÓL!\n"
    ...
    def generate_card_message(self, match: Dict, event: Dict, card_type: str) -> str:
        """Generate card message with improved format"""
        home_team, away_team = self.extract_team_names(match)
        league_name = self.extract_league_name(match)
        ...
        # FIXED: Improved message format with proper team display
