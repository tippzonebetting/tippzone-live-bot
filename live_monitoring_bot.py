...
class LiveMonitoringBot:
    def __init__(self):
        ...
        # CHANNEL FIX: Remove hardcoded default, use only ENV variable
        self.channel_id = os.getenv('TELEGRAM_CHANNEL_ID')  # pl. @tippzone_live vagy -100...
        logger.info(f"Channel env loaded: {self.channel_id}")
        self.bot = None
        ...
        # Extended card event IDs including second yellow
    ...
    def calculate_progressive_score(self, match: Dict, current_event: Dict) -> tuple:
        """Calculate progressive score with own goal handling and precise sorting"""
    ...
    def extract_events(self, match: Dict) -> List[Dict]:
        ...
        for event in events:
            ...
            # Extra fallback for team identification
            ...
            # Log "Ismeretlen" team cases for diagnostics
            if team_name == 'Ismeretlen':
    ...
    def generate_goal_message(self, match: Dict, event: Dict) -> str:
        """Generate goal message with progressive score calculation and TippZone style"""
        home_team, away_team = self.extract_team_names(match)
        
        # Use progressive score calculation instead of current match score
        home_score, away_score = self.calculate_progressive_score(match, event)
        
        league_name = self.extract_league_name(match)
        state = match.get('state', {})
        
        # TIPPZONE STYLE: Clean format with progressive score and 🆚 icon
        message = f"⚽ GÓL!\n"
    ...
    def generate_card_message(self, match: Dict, event: Dict, card_type: str) -> str:
        """Generate card message with consistent formatting and TippZone style"""
        home_team, away_team = self.extract_team_names(match)
        league_name = self.extract_league_name(match)
        state = match.get('state', {})
        ...
        # TIPPZONE STYLE: Consistent format with goal messages and 🆚 icon
    ...
    async def monitoring_loop(self):
        ...
        while True:
            try:
                ...
                # Enhanced memory cleanup for finished matches
                for match in filtered_matches:
                    ...
                    if st in ('FT', 'AET', 'FT_PEN', 'FT_ET', 'ENDED'):
                        ...
                        # Clean up last_sent dict as well
    ...
    async def start(self):
        """Start the monitoring bot with enhanced validation"""
        ...
        # CHANNEL FIX: Fail-fast channel validation
        if not self.channel_id:
            logger.error("TELEGRAM_CHANNEL_ID hiányzik – állj le.")
            return

        # Sanity check for channel ID format
        cid = str(self.channel_id)
        if cid.isdigit() and not cid.startswith("-100"):
            logger.warning("FIGYELEM: numerikus TELEGRAM_CHANNEL_ID, de nem '-100'-zal kezdődik. "
                           "Csatorna esetén használd az @tippzone_live formát vagy a -100... ID-t.")
